 ...
 * The x axis has the positive direction going to the right, and the y axis has the
 * positive direction going down.
 *
 ...
 *
 * A **transparent** tile is a tile which allows light to pass through it. This is
 * primarily used by FOV algorithms, to determine which tiles an entity can see. A
 * non-transparent tile will be visible, but will block light, so no tiles behind it will
 * be visible when casting a light ray.
 */
class SquareGridMap
{
public:
     ...
     * be used instead, as setting a cost greater than zero will make the tile walkable
     * too.
     *
     * @param pos A 2D position.
    ...
    /**
     * Overloaded version of @ref set_transparency(const Position2D& pos, bool
     * transparent) const which takes the coordinates of the position as arguments.
     *
     * @param x The x coordinate of a 2D position.
     * @param y The y coordinate of a 2D position.
     *
    ...
    /**
     * Returns a vector with the walkable tiles adjacent to the tile at the given
     * position.
     *
     * Only tiles within the boundaries of the map and that are walkable will be
     * considered as neighbours. If the map allows diagonals (see constructor), tiles
     * positioned diagonally to the tile at the given position will also be considered.
     *
     * @param pos A 2D position.
     *
