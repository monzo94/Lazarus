...
class TestSystem : public Updateable,
                   public EventListener<TestEvent>,
                   public EventListener<TestEvent2>
{
public:
...
TEST_CASE("entity management")
{
    ...
    SECTION("entities_with_components")
    {
        ...
        entities = engine.entities_with_components<TestComponent, TestComponent2>();
        REQUIRE(entities.size() == 1);
        REQUIRE(entities[0]->get_id() == id1);
    }
    SECTION("apply_to_each with lambda")
    {
        ...
        engine.apply_to_each<TestComponent>(
            [](Entity *ent, TestComponent *comp) { comp->num++; });
...
TEST_CASE("event management")
{
    ...
    SECTION("emitting and receiving events from the engine with subscribers")
    {
        ...
        engine.emit(event);
        REQUIRE(x == 10);
        // Adding more of the same system to the same event type will not create a new
        // system
        REQUIRE_NOTHROW(engine.add_system<TestSystem, TestEvent>());
        REQUIRE_NOTHROW(engine.add_system<TestSystem, TestEvent>());
        engine.emit(event);
